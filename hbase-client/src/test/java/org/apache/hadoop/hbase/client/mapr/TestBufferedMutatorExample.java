 ...
 */
public class TestBufferedMutatorExample extends Configured implements Tool {
  ...
  public static class SinglePut implements Callable<Void> {
    ...
    private void putarow(int n) throws Exception {
      ...
      p.addColumn(CF1, Bytes.toBytes("qual1"), Bytes.toBytes("value1"));
      mutator.mutate(p);
  ...
  public int run(String[] argv) throws InterruptedException, ExecutionException, TimeoutException {
    ...
    final BufferedMutator.ExceptionListener listener = new BufferedMutator.ExceptionListener() {
      ...
      public void onException(RetriesExhaustedWithDetailsException e, BufferedMutator mutator) {
        for (int i = 0; i < e.getNumExceptions(); i++) {
          Row row = e.getRow(i);
          LOG.info("Failed to sent put "+ i + ":" + row + ".");
        }
      }
    };
    ...
    //
      ...
      workerPool.shutdown();
      LOG.info("WorkerPool shutdown successfully");
      mutator.close();
      LOG.info("Mutator close successfully");
      conn.close();
      LOG.info("Connection close successfully");

    } catch (IOException e) {
      // exception while creating/destroying Connection or BufferedMutator
      LOG.info("exception while creating/destroying Connection or BufferedMutator");
      e.printStackTrace();
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new TestBufferedMutatorExample(), args);
  }
}
